...
This is a stepped progress bar for IOS. It is written upon ABSteppedProgressBar. You can modify most of the design. Text can be positioned inside the circles or on top or bottom of them or if the user wants, at all the three places. Please refer to usage section for more details.
...
```
## Usage
I improved upon the previous plugin because I had an extra state to cater to in the progress bar other than selected and unselected, Last State. It specifies, the distance user has already covered on the progress bar. For example, in my case, it was specifying properties of a product in a particular oder. So, if the user has completed till step 3, and he goes back to step 1 to make some modification, progres bar should indicate separately the step 3 as last state and step 1 as selected state. It is completely optional here. If you want to use last state, you can simply state your wish through useLastState parameter and specify completedTillIndex as the last state. Progress bar does not keep tract of the last state.
    ...
    CAUTION: If your background view has any other color than white. Please specify it in viewBackgroundColor. Otherwise you will keep seeing white color in between.
